...
- [ ] **GUI Enhancements (Bootstrap)**
  - [ ] Implement responsive sidebar navigation (utilizing `sidebars.js`/`sidebars.css`).
...
- [ ] **Module Organization**
  ...
  - [ ] Implement a dynamic loader in `modules.ahk` to include all scripts in the `Modules/` directory automatically or via an array.
- [ ] **State & Configuration Management (mode.ini)**
  - [ ] Write a robust function to read/write settings to `mode.ini`.
  - [ ] Sync `mode.ini` settings with the Webview interface on load so checkboxes/switches reflect the actual saved state.
- [ ] **AHK <-> JS Communication**
  - [ ] Standardize the message-passing system using `WebViewToo.ahk` and `Promise.ahk`.
  - [ ] Use `ComVar.ahk` securely if passing large objects or arrays between AHK and JS is required.

## 🧹 3. Refactoring & Cleanup
*Items to keep the codebase clean and avoid technical debt.*
- [ ] **Code Refactoring**
  - [ ] Add automatic config making into 'mode.ini' (for each file inside main.ahk detecting #include) if it doesn't exist, instead of relying on manual creation and hard code inside the main script.
  - [ ] Consolidate overlapping logic between `AltArrow`, `CursLock`, and `FkeysNumpad`.
  - [ ] Add standard Header Comments and inline documentation to all custom AHK files.
- [ ] **Asset Management**
  - [ ] Optimize Bootstrap assets (if custom icons or fonts are added to `Pages/Bootstrap/fonts/`).
